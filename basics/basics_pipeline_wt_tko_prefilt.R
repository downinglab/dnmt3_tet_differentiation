...
samples <- c("wt_rep1", "wt_rep2", "tko_rep1", "tko_rep2")
...
filtered_dko <- cbind(filtered_list$tko_rep1, filtered_list$tko_rep2)
...
basics_data_dko <- newBASiCS_Data(
  filtered_dko, 
  BatchInfo = rep(c(1, 2), c(ncol(filtered_list$tko_rep1), ncol(filtered_list$tko_rep2)))
)
...
# Save results
path_out <- "tko_wt_basics_results.csv"
